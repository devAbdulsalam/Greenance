...
const UpdateProfile = ({ user, token, setEdit, setProfile, setLoading }) => {
	...
	const handleSubmit = (e) => {
		...
		axios
			.post('http://localhost:4000/user/update', data, {
				headers: {
			...
			.catch((err) => {
				...
				setLoading(false);
				setError(
					err.response?.data.error || err.message || err.response || err
				);
			});
	};
	return (
		...
		>
			<div className="mr-auto place-self-center lg:col-span-7 w-full md:text-left text-center px-2">
				<div className="flex items-center px-2 w-full">
					<label htmlFor="name" className="w-3/12 md:w-1/12">
						Name
					</label>
					<input
				...
				<div className="flex items-center px-2">
					<label htmlFor="phone" className="w-3/12 md:w-1/12">
						Phone
					</label>
					<input
						value={phone}
				...
				<div className="flex items-center px-2">
					<label htmlFor="email" className="w-3/12 md:w-1/12">
						Email
					</label>
					<input
						value={email}
				...
				<div className="flex items-center px-2">
					<label htmlFor="address" className="w-3/12 md:w-1/12">
						Address
					</label>
					<input
						value={address}
			...
			<div className="py-2 px-4 w-full flex justify-center">
				<button
					type="submit"
					className="p-2 w-full md:max-w-xl px-6 justify-center rounded-md text-xl bg-green-500 hover:bg-green-600 text-white"
				>
					Update Profile
				</button>
			</div>
