...
const UpdateImage = ({ user, token, setEdit, setProfile, setLoading }) => {
	...
	const handleSubmit = (e) => {
		...
		axios
			.post('http://localhost:4000/user/updateImage', image, {
				headers: {
		...
		>
			<div className="flex pt-2 w-full flex-col place-items-center">
				<label htmlFor="image" className="w-full mx-auto rounded-full">
					<img
						className="w-32 cursor-pointer h-32 mx-auto"
						src={Img}
						alt="my-pic"
					/>
					<p className="cursor-pointer text-center p-2 text-lg">
						{imageName || 'Choose a file'}
					</p>
				</label>
				<input
					className="absolute -z-10 top-14 self-center invisible"
					type="file"
					id="image"
					name="image"
